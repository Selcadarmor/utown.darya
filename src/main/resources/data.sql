...
-- 5. Ресторан (с привязкой к администратору и адресу)
...
-- 6. Пользователь - клиент (с корзиной, файлом и адресом через addresses.client_id)
INSERT INTO users (
    id, is_active, password, phone_number, created_at, updated_at, dtype, full_name, default_address, file_id, cart_id
) VALUES (
             1, TRUE, '$2a$10$haz8tbPPooj63WjWdL.mEup/VLBHop6ZA9L8B1e2zHAvBpBnq14Xe', '+79990001127', NOW(), NOW(), 'client', 'Иван Иванов', 1, 1, 1
         )
...
-- 8. Пользователь - RestaurantAdmin с ID=101
INSERT INTO users (
...
-- 9. Связь user_roles для админа (100 - ROLE_ADMIN)
INSERT INTO user_roles (user_id, role_id) VALUES
    (100, 1)
ON DUPLICATE KEY UPDATE user_id=VALUES(user_id);

-- 10. Связь user_roles для ресторанного админа (101 - ROLE_RESTAURANT_ADMIN)
INSERT INTO user_roles (user_id, role_id) VALUES
    (101, 3)
ON DUPLICATE KEY UPDATE user_id=VALUES(user_id);

-- 11. Категории ресторана
...
-- 12. Связь ресторан - категория (ManyToMany)
...
-- 18. Связь элементов с заказом блюд
INSERT INTO dish_to_order_elements (dish_to_order_id, element_id) VALUES (1, 1), (1, 2);

-- 19. Доставка (связь с рестораном по restaurant_id)
...
-- 22. Заказы (с client_id = 1 и restaurant_id = 1)
INSERT INTO orders (
